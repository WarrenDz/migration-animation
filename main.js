...
mapElement.addEventListener("arcgisViewReadyChange", (event) => {
  ...
  if (event.target.ready) {
    ...
    async function updateMapChoreography() {
      await applyTrackRender(); // Wait for the track renderer to be applied
      toggleLayerVisibility(hashLayersOff, hashLayersOn, layers); // Toggle layer visibility (synchronous)
      updateTimeSlider(); // Update the time slider (synchronous)
      updateMapBookmark(); // Update the map bookmark (synchronous)
    }
    ...
    async function applyTrackRender() {
      if (trackLayer) {
        await trackLayer.when();
    ...
    function updateTimeSlider() {
        ...
        timeSlider.playRate = choreographyMapping[hash].playRate; // Set the play rate to in milliseconds per frame
